...
function Funcionarios() {
    ...
    const conteudoHTML = `
      <html>
        ...
        <body>
          ...
          <table>
            <thead><tr><th style="width: 15%">Matrícula</th><th style="width: 35%">Nome</th><th style="width: 25%">Departamento</th><th style="width: 25%">Cargo</th></tr></thead>
            <tbody>
              ${funcionariosFiltrados.map(f => `<tr><td class="matricula">${f.matricula}</td><td class="nome">${f.nome}</td><td><span class="badge">${f.departamento.nome}</span></td><td>${f.funcao.nome}</td></tr>`).join('')}
            </tbody>
          </table>
          <div class="footer"><div class="assinatura-box">Gerente de RH</div><div class="assinatura-box">Responsável</div></div>
  ...
  function salvarFuncionario() {
    ...
    if (funcSelecionado) {
      ...
      setPaginaAtual(Math.ceil((funcionarios.length + 1) / itensPorPagina));
    }
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg border border-gray-100 animate-fade-in max-w-full">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-200">
            {funcionariosVisiveis.length > 0 ? (
              ...
              <tr>
                <td colSpan="5" className="p-8 text-center text-gray-500">Nenhum funcionário encontrado.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {/* --- MODO MOBILE/TABLET (Cards) - Visível até LG --- */}
            ...
            <div key={func.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm relative">
                ...
                <div className="grid grid-cols-2 gap-3 mt-4 pt-3 border-t border-gray-100">
                    <button 
                        ...
                        className="flex items-center justify-center gap-2 py-2 text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg text-sm font-bold transition"
                    >
                        ✏️ Editar
                    </button>
                    <button 
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                ...
                <select className={`w-full p-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white ${!formDepartamento ? 'bg-gray-50 cursor-not-allowed' : ''}`} value={formFuncao} onChange={(e) => setFormFuncao(e.target.value)} disabled={!formDepartamento}>
                  <option value="">{!formDepartamento ? "Selecione o departamento" : "Selecione a função"}</option>
            ...
            <div className="bg-gray-50 px-6 py-4 flex justify-end gap-3 shrink-0 border-t">
              ...
              <button onClick={salvarFuncionario} className="px-4 py-2 bg-blue-700 text-white font-bold rounded-lg hover:bg-blue-800 shadow-md transition">Salvar</button>
            </div>
