...
const mockFornecedoresInicial = [
  { id: 1, nome: "3M do Brasil", cnpj: "45.985.371/0001-08", contato: "vendas@3m.com" },
  { id: 2, nome: "Bracol Calçados", cnpj: "12.345.678/0001-90", contato: "comercial@bracol.com.br" },
];

const mockDepartamentosInicial = [
  { id: 1, nome: "Produção", cor: "bg-blue-100 text-blue-700" },
  { id: 2, nome: "Logística", cor: "bg-orange-100 text-orange-700" },
  { id: 3, nome: "Segurança do Trabalho", cor: "bg-green-100 text-green-700" },
];
...
function Administracao() {
  ...
  function adicionarDepartamento() {
    ...
    const item = { id: Date.now(), nome: novoDepto, cor: "bg-gray-100 text-gray-700" }; 
  ...
  // --- FUNÇÕES DE EXCLUSÃO ---
  function removerItem(id, setter, lista) {
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg border border-gray-100 animate-fade-in max-w-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">Gerencie tabelas auxiliares e cadastros do sistema.</p>
      </div>
        ...
        <div className="animate-fade-in">
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
              ...
              <div>
                <button onClick={adicionarFornecedor} className="w-full bg-emerald-600 text-white font-bold py-2 rounded hover:bg-emerald-700 transition text-sm">
                  + Cadastrar
                </button>
              </div>
            </div>
                ...
                {fornecedores.map(f => (
                  <tr key={f.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-3 text-center">
                      <button onClick={() => removerItem(f.id, setFornecedores, fornecedores)} className="text-red-500 hover:text-red-700 font-bold text-xs">Excluir</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="animate-fade-in">
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6 flex flex-col md:flex-row gap-3 items-end">
            <div className="flex-1 w-full">
              ...
              <input className="w-full p-2 border rounded focus:ring-2 focus:ring-slate-500 outline-none text-sm" 
                value={novoDepto} onChange={e => setNovoDepto(e.target.value)} placeholder="Ex: Recursos Humanos" />
            </div>
            <button onClick={adicionarDepartamento} className="w-full md:w-auto px-6 bg-emerald-600 text-white font-bold py-2 rounded hover:bg-emerald-700 transition text-sm">
              + Adicionar
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {departamentos.map(d => (
              <div key={d.id} className="flex justify-between items-center p-3 border rounded-lg bg-white shadow-sm hover:shadow-md transition">
                ...
                <button onClick={() => removerItem(d.id, setDepartamentos, departamentos)} className="text-gray-400 hover:text-red-500 transition">🗑️</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
              ...
              <div className="md:col-span-1">
                ...
                <input className="w-full p-2 border rounded focus:ring-2 focus:ring-slate-500 outline-none text-sm" 
                  value={novoCargo.nome} onChange={e => setNovoCargo({...novoCargo, nome: e.target.value})} placeholder="Ex: Auxiliar Administrativo" />
              </div>
              <div>
                <button onClick={adicionarCargo} className="w-full bg-emerald-600 text-white font-bold py-2 rounded hover:bg-emerald-700 transition text-sm">
                  + Salvar Cargo
                </button>
              </div>
            </div>
                ...
                {cargos.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-slate-50">
                      ...
                      <td className="p-3 text-center">
                        <button onClick={() => removerItem(c.id, setCargos, cargos)} className="text-red-500 hover:text-red-700 font-bold text-xs">Excluir</button>
                      </td>
                    </tr>
                  )
                })}
